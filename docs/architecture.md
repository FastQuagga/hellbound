...
Стенные текстуры-панно 64×64 (эксперимент: пропсы) — `js/assets/sprites_props.js`,
`export function generatePropTextures(base?): Map<string, HTMLCanvasElement>`
(`base` — необязательный Map базовых текстур; края панно = края базовой текстуры,
стыкуются бесшовно с соседними клетками той же базы):

| Ключ | База | Описание |
|---|---|---|
| `PAINT_TECH` | `TECH1` | настенная панель-экран со схемой базы (зелёные линии) |
| `PAINT_UAC` | `TECH1` | металлический плакат корпорации UAC с предупреждающей полосой |
| `VENT` | `TECH1` | утопленная вентиляционная решётка с жалюзи и потёками копоти |
| `PAINT_DEMON` | `MARBLE` | адский триптих: три барельефа с демоном и черепами |
| `PAINT_PENTA` | `BRICKRED` | светящаяся пентаграмма, начертанная на кладке |
...
Дополнительный декор (эксперимент: пропсы) — `js/assets/sprites_props.js`,
`export function generateSprites(): Map<string, HTMLCanvasElement>`.
Канвас 64×64, основание на нижней кромке, кроме отмеченных:

| Ключ | Описание |
|---|---|
| `skull_pile` | горка черепов с костями в основании |
| `bones` | разбросанные кости: грудная клетка, череп на боку |
| `rubble` | обломки бетона с ржавой арматурой |
| `shells_spent` | **64×16**, россыпь стреляных гильз (плоская, на полу) |
| `blood_pool` | **64×16**, лужа крови с рваным краем (плоская, на полу) |
| `cable_coil` | моток толстого кабеля с разъёмом |
| `monitor_broken` | разбитый ЭЛТ-монитор на тумбе, трещина-паутина |
| `candle0..1` | свеча в лужице воска, 2 кадра пламени (bright) |
| `lamp_red0..1` | аварийная красная лампа на стойке, 2 кадра (ярко/тускло, bright) |
| `hanging_chain` | цепь с крюком, подвес у ВЕРХНЕЙ кромки, низ прозрачный |
| `gore_hang` | подвешенная на крюке туша, подвес у ВЕРХНЕЙ кромки |
| `crate_small` | одиночный армейский ящик с маркировкой (блокирующий, radius 0.3) |
